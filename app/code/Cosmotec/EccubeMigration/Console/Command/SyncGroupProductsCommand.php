...
class SyncGroupProductsCommand extends Command
{
    ...
    public function __construct(
        private readonly ItemSync $sync,
        private readonly EccubeConfigProviderInterface $config,
        private readonly ExecuteModeResolver $executeModeResolver,
        private readonly State $appState
    ) {
        parent::__construct('cosmotec:eccube:sync:group-products');
    }

    protected function configure(): void
    {
        $this->setDescription('Re-sync EC-CUBE items (Grouped Products) modified since the last successful import/sync. Dry-run unless --execute is passed.');
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $runId = 'sync-item-' . date('Ymd-His') . '-' . substr(bin2hex(random_bytes(4)), 0, 8);

        $output->writeln(sprintf('Starting group product sync (run %s)%s', $runId, $dryRun ? ' [DRY RUN]' : ''));

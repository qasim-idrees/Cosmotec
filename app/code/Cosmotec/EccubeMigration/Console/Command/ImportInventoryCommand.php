...
class ImportInventoryCommand extends Command
{
    ...
    public function __construct(
        ...
        parent::__construct('cosmotec:eccube:import:inventory');
    }

    protected function configure(): void
    {
        $this->setDescription('Reconcile EC-CUBE stock (dtb_product.stock_quantity) into Magento via MSI source items for already-imported products. Dry-run unless --execute is passed.');
        ...
        $this->addOption(self::OPTION_RESUME, null, InputOption::VALUE_NONE, 'Resume a previous run; unchanged records are always skipped regardless of this flag.');
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $runId = 'inv-' . date('Ymd-His') . '-' . substr(bin2hex(random_bytes(4)), 0, 8);

        $output->writeln(sprintf(
            'Starting inventory import (run %s)%s%s',
            $runId,
            $dryRun ? ' [DRY RUN]' : '',
            $resume ? ' [RESUME]' : ''
        ));

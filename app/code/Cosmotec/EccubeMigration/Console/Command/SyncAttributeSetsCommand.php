...
use Cosmotec\EccubeMigration\Model\Sync\AttributeSetSync;
 ...
 */
class SyncAttributeSetsCommand extends Command
{
    ...
    public function __construct(
        private readonly AttributeSetSync $sync,
        private readonly EccubeConfigProviderInterface $config,
        private readonly ExecuteModeResolver $executeModeResolver
    ) {
        parent::__construct('cosmotec:eccube:sync:attribute-sets');
    }

    protected function configure(): void
    {
        $this->setDescription('Re-sync Magento Attribute Sets from EC-CUBE top-level categories (always incremental via content hash). Dry-run unless --execute is passed.');
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $runId = 'sync-attrset-' . date('Ymd-His') . '-' . substr(bin2hex(random_bytes(4)), 0, 8);

...
use Cosmotec\EccubeMigration\Model\Sync\AttributeSync;
 ...
 *
 * Deliberately uses the newer --execute-required safety convention
 * (matching import:attributes/import:images), not the older sync:*
 * commands' writes-by-default-unless---dry-run pattern, per explicit
 * instruction to keep every migration command dry-run by default this
 * round.
 */
class SyncAttributesCommand extends Command
{
    ...
    public function __construct(
        private readonly AttributeSync $sync,
        private readonly EccubeConfigProviderInterface $config,
        private readonly ExecuteModeResolver $executeModeResolver
    ) {
        parent::__construct('cosmotec:eccube:sync:attributes');
    }

    protected function configure(): void
    {
        $this->setDescription('Re-sync EC-CUBE specifications/options into Magento attributes (always incremental via content hash). Dry-run unless --execute is passed.');
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $runId = 'sync-attr-' . date('Ymd-His') . '-' . substr(bin2hex(random_bytes(4)), 0, 8);

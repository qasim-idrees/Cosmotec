...
use Cosmotec\EccubeMigration\Model\Import\ProductAttributeSetAssignmentImporter;
 ...
 * set resolved from its owning item's EC-CUBE category tree via
 * AttributeSetResolver. Must run before import:product-attribute-values on
 * any product still on the catalog's disposable staging default set - see
 * BUILD_STATUS.md Round 31.
 */
class AssignProductAttributeSetsCommand extends Command
{
    ...
    public function __construct(
        ...
        parent::__construct('cosmotec:eccube:assign:product-attribute-sets');
    }

    protected function configure(): void
    {
        $this->setDescription('Reassign Simple Products to their EC-CUBE-category-derived Magento attribute set. Dry-run unless --execute is passed.');
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($dryRun) {
            ...
            $output->writeln('<info>EXECUTING — Simple Product attribute-set assignments will be changed.</info>');
        }

        $output->writeln('');

        $batchSizeOption = $input->getOption(self::OPTION_BATCH_SIZE);
        $runId = 'productsetassign-' . date('Ymd-His') . '-' . substr(bin2hex(random_bytes(4)), 0, 8);
        ...
        $output->writeln('<comment>"Needs review" items have no resolvable EC-CUBE top-level category chain (via their owning item) and need an explicit fallback bucket before they can be assigned.</comment>');

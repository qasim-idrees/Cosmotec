...
class SyncSimpleProductsCommand extends Command
{
    ...
    public function __construct(
        private readonly ProductSync $sync,
        private readonly EccubeConfigProviderInterface $config,
        private readonly ExecuteModeResolver $executeModeResolver,
        private readonly State $appState
    ) {
        parent::__construct('cosmotec:eccube:sync:simple-products');
    }

    protected function configure(): void
    {
        $this->setDescription('Re-sync EC-CUBE products (Simple Products) modified since the last successful import/sync. Dry-run unless --execute is passed.');
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $runId = 'sync-prod-' . date('Ymd-His') . '-' . substr(bin2hex(random_bytes(4)), 0, 8);

        $output->writeln(sprintf('Starting simple product sync (run %s)%s', $runId, $dryRun ? ' [DRY RUN]' : ''));

...
class ImportGroupProductsCommand extends Command
{
    ...
    public function __construct(
        ...
        parent::__construct('cosmotec:eccube:import:group-products');
    }

    protected function configure(): void
    {
        $this->setDescription('Import EC-CUBE items into Magento as Grouped Product shells (no child links yet). Dry-run unless --execute is passed.');
        ...
        $this->addOption(self::OPTION_RESUME, null, InputOption::VALUE_NONE, 'Resume a previous run; already-imported items are always skipped regardless of this flag.');
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $runId = 'item-' . date('Ymd-His') . '-' . substr(bin2hex(random_bytes(4)), 0, 8);

        $output->writeln(sprintf(
            'Starting group product import (run %s)%s%s',
            $runId,
            $dryRun ? ' [DRY RUN]' : '',
            $resume ? ' [RESUME]' : ''
        ));

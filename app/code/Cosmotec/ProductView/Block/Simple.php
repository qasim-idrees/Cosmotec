...
class Simple extends Template implements IdentityInterface
{
    protected $registry;
    ...
    public function getCacheKeyInfo()
    {
        $product = $this->getProduct();
        return [
            'COSMOTEC_SIMPLE_PRODUCT_INFO_BLOCK',
    ...
    public function isConfigurableProduct(Product $product)
    {
     ...
     /**
     * Get parent product description from a simple (child) product.
     *
     * @param \Magento\Catalog\Model\Product $childProduct
     * @return string|null
     */
    public function getParentDescriptionFromChild(Product $childProduct)
    {
        if ($this->isSimpleProduct($childProduct)) {
            // Get parent IDs (can return multiple IDs normally, pick first)
            $parentIds = $this->groupedType->getParentIdsByChild($childProduct->getId());
            ...
            if (!empty($parentIds)) {
                ...
                // Return the description (or any other attribute)
                return $parentProduct->getShortDescription();
            }
        }
     ...
     * @return string|null
     */
    public function getParentAttributeFromChild(Product $childProduct, $attributeCode)
    {
        if ($this->isSimpleProduct($childProduct) && !empty($attributeCode)) {
            // Get parent IDs (can return multiple IDs normally, pick first)
            $parentIds = $this->groupedType->getParentIdsByChild($childProduct->getId());
            ...
            if (!empty($parentIds)) {
                ...
                // Return the description (or any other attribute)
                return $parentProduct->getData($attributeCode);
            }
        }
